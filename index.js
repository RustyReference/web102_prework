...
let numContributions = GAMES_JSON.reduce((acc, game) => acc + game.backers, 0);
...
gamesCard.innerHTML = `${GAMES_JSON.reduce((acc, _) => acc + 1, 0)}`
...
const displayStr = `A total of $${numGames.toLocaleString("en-US")} 
    has been raised for ${numGames > 1 ? "games" : "game"}. 
    Currently, ${numUnfunded} ${numUnfunded > 1 ? "games" : "game"}
    ${numUnfunded > 1 ? "remain" : "remains"} unfunded. 
...
let topPledgedName = document.createElement("div");
topPledgedName.innerHTML = first.name;
firstGameContainer.appendChild(topPledgedName);
